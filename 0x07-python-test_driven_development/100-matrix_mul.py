#!/usr/bin/python3
""" module for matrix multipilication"""
...
def matrix_mul(m_a, m_b):
    ...
    if False in [isinstance(listx, list) for listx in m_a]:
        raise TypeError("m_a must be a list of lits")
    ...
    if len(set(len(listx) for listx in m_a)) > 1:
        raise TypeError("each row of m_a must shiuld be of the same size ")
    if len(set(len(listx) for listx in m_b)) > 1:
        raise TypeError("each row of m_b must shiuld be of the same size ")
